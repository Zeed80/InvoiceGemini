...
class FieldManager:
    ...
    def __init__(self):
        ...
        if os.path.exists(self.fields_config_path):
            print("[FIELD_MANAGER] Найден файл пользовательских настроек - загружаем только их")
            self._load_custom_fields()
        else:
            print("[FIELD_MANAGER] Файл пользовательских настроек не найден - создаем поля по умолчанию")
