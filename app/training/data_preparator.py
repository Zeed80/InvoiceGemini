...
class TrainingDataPreparator:
    ...
    def __init__(self, app_config, ocr_processor, gemini_processor):
        ...
        # 🔍 PDF анализатор для интеллектуальной обработки
        try:
            self.pdf_analyzer = PDFTextAnalyzer(logger=None)  # Используем None для логгера
            self._log("✅ PDF анализатор инициализирован успешно")
        except Exception as e:
            self._log(f"⚠️ Ошибка инициализации PDF анализатора: {e}")
            self.pdf_analyzer = None
            
        # 🚀 Система повышения качества данных до 100%
        try:
            from .data_quality_enhancer import DataQualityEnhancer
            self.quality_enhancer = DataQualityEnhancer(logger=self.logger)
            self._log("✅ DataQualityEnhancer инициализирован - теперь стремимся к 100% качеству!")
        except Exception as e:
            self._log(f"⚠️ Ошибка инициализации DataQualityEnhancer: {e}")
            self.quality_enhancer = None
        
        # Кэш для данных из текстового слоя PDF
        self._pdf_text_blocks = None
    ...
    def _mark_matching_words(self, words, field_value, field_type, labels):
        """
        Маркирует слова, соответствующие значению поля.
        ...
        Returns:
            bool: True если были найдены совпадения, False в противном случае
        """
        ...
        if field_value_clean in skip_values:
            self._log(f"Пропускаем служебное значение для поля '{field_type}': '{field_value}'")
            return False
        
        # Разбиваем значение поля на токены (слова)
        field_tokens = field_value.split()
        
        # Пытаемся найти совпадения для каждого токена поля
        for i in range(len(words)):
            # Окно поиска - проверяем несколько последовательных слов начиная с i
            for window_size in range(min(len(field_tokens), 5), 0, -1):
                if i + window_size <= len(words):
                    # Создаем окно из слов
                    window_words = [word.lower() for word in words[i:i+window_size]]
                    window_text = " ".join(window_words)
                    
                    # Проверяем, есть ли подходящее совпадение
                    if self._is_matching_text(window_text, field_value):
                        # Помечаем первое слово как B-{field_type} (Begin)
                        labels[i] = f"B-{field_type}"
                        
                        # Помечаем остальные слова как I-{field_type} (Inside)
                        for j in range(i+1, i+window_size):
                            labels[j] = f"I-{field_type}"
                            
                        # Выводим информацию о сопоставлении
                        self._log(f"Найдено точное совпадение для '{field_type}': '{field_value}' => '{window_text}' (слова {i}-{i+window_size-1})")
                        
                        # Возвращаем True, так как нашли совпадение
                        return True
                        
        # Если по всему тексту не нашли точного совпадения, пробуем частичное совпадение
        if field_type not in " ".join(labels):
            self._log(f"Поиск частичного совпадения для поля '{field_type}': '{field_value}'")
            
            # Ищем частичные совпадения (для полей с цифрами или специфичным форматом)
            for i, word in enumerate(words):
                # Проверяем слово и его окрестности
                neighborhood = []
                for j in range(max(0, i-2), min(len(words), i+3)):
                    neighborhood.append(words[j].lower())
                ...
                # Ищем подстроки или числовые совпадения
                if field_value in word.lower():
                    found_match = True
                    match_reason = f"значение '{field_value}' содержится в слове '{word.lower()}'"
                elif word.lower() in field_value:
                    found_match = True
                    match_reason = f"слово '{word.lower()}' содержится в значении '{field_value}'"
                elif self._has_common_numbers(word.lower(), field_value):
                    found_match = True
                    match_reason = f"общие числа между '{word.lower()}' и '{field_value}'"
                elif field_value in neighborhood_text:
                    found_match = True
                    match_reason = f"значение '{field_value}' найдено в окрестности: '{neighborhood_text}'"
                
                if found_match:
                    # Уже размечено? Пропускаем
                    if labels[i] != "O":
                        continue
                        
                    # Помечаем текущее слово
                    labels[i] = f"B-{field_type}"
                    
                    self._log(f"Найдено частичное совпадение для '{field_type}': {match_reason}")
                    
                    # Ищем потенциальные продолжения тем же способом
                    for j in range(i+1, min(len(words), i+4)):
                        ...
                        if (next_word in field_value or 
                            self._has_common_numbers(next_word, field_value) or
                            any(part in field_value for part in next_word.split())):
                            labels[j] = f"I-{field_type}"
                            self._log(f"  + добавлено продолжение '{next_word}' для поля '{field_type}'")
                        else:
                            # Остановка, если нашли слово, которое точно не относится к полю
                            break
                    
                    # Нашли совпадение, возвращаем успех
                    return True
        
        # Если не нашли совпадений, сообщаем об этом
    ...
    def _convert_to_images_modern(self, file_path) -> List[Image.Image]:
        ...
        - PDF с текстовой подложкой: извлекает текст напрямую 
        - PDF без текста: конвертирует в изображения для OCR
        - Изображения: загружает как есть
        """
        import traceback
        import os
            ...
            if file_path.suffix.lower() in supported_image_formats:
                ...
                try:
                    if not self.pdf_analyzer:
                        raise Exception("PDF анализатор не инициализирован")
    ...
    def _extract_fields_from_pdf_text_layer(self) -> Dict:
        """
        Извлекает поля из данных текстового слоя PDF
        Использует интеллектуальный анализ расположения текста для извлечения ключевых полей
        """
        try:
            if not self._pdf_text_blocks:
                return {}
            
            self._log(f"     📄 Анализ {len(self._pdf_text_blocks)} текстовых блоков из PDF...")
            
            # Собираем весь текст для общего анализа
            all_text = ""
            text_positions = []
            
            for block in self._pdf_text_blocks:
            ...
            # Используем Gemini для анализа текста если доступен
            if self.gemini_processor:
                self._log(f"     🤖 Анализ текста через Gemini...")
                
                # Создаем промпт для извлечения данных из текста
                prompt = f"""
                Проанализируй следующий текст из счета и извлеки ключевые поля.
                ...
                try:
                    import json
                    # Здесь должен быть вызов Gemini API для анализа текста
                    # Пока используем простое извлечение ключевых полей
                    extracted_fields = self._extract_fields_with_simple_patterns(all_text)
                    self._log(f"     ✅ Извлечено полей из текстового слоя: {len(extracted_fields)}")
                    return extracted_fields
                    
                except Exception as e:
                    self._log(f"     ⚠️ Ошибка анализа через Gemini: {e}")
            
            # Fallback к простому извлечению по паттернам
            self._log(f"     🔍 Используем простое извлечение по паттернам...")
            return self._extract_fields_with_simple_patterns(all_text)
            
        except Exception as e:
            self._log(f"     ❌ Ошибка извлечения полей из текстового слоя: {e}")
            return {}
    
    def _extract_fields_with_simple_patterns(self, text: str) -> Dict:
        """Простое извлечение полей по регулярным выражениям"""
        import re
        from datetime import datetime
        ...
        try:
            ...
            if match:
                fields['company_inn'] = match.group(1)
            
            # Сумма
            amount_patterns = [
                r'итого:?\s*(\d+(?:[.,]\d+)?)\s*руб',
                r'сумма:?\s*(\d+(?:[.,]\d+)?)\s*руб',
                r'к\s*оплате:?\s*(\d+(?:[.,]\d+)?)\s*руб',
                r'всего:?\s*(\d+(?:[.,]\d+)?)\s*руб'
            ]
            ...
            # Название компании (ищем ООО, ИП и т.д.)
            company_patterns = [
                r'(ООО\s+["\']?[^"\']+["\']?)',
                r'(ОАО\s+["\']?[^"\']+["\']?)',
                r'(ЗАО\s+["\']?[^"\']+["\']?)',
            ...
            for pattern in company_patterns:
                ...
                if match:
                    ...
                    if len(company_name) > 3:  # Фильтруем слишком короткие названия
                        fields['company_name'] = company_name
                        break
            
            self._log(f"     📊 Извлеченные поля: {list(fields.keys())}")
            return fields
            
        except Exception as e:
            self._log(f"     ❌ Ошибка извлечения по паттернам: {e}")
            return {}
    ...
    def _create_donut_annotation_modern(self,
        ...
        try:
            ...
            # 🚀 Консенсус-алгоритм для максимального качества Donut
            if self.quality_enhancer:
                self._log(f"      🎯 КАЧЕСТВЕННОЕ ИЗВЛЕЧЕНИЕ DONUT: Консенсус всех методов...")
                
                # Собираем текст из всех источников
                all_text = ""
                if hasattr(self, '_pdf_text_blocks') and self._pdf_text_blocks:
                    all_text = " ".join([block.get('text', '') for block in self._pdf_text_blocks])
                ...
                if annotation_method == "gemini" and self.gemini_processor:
                    gemini_result = self._extract_fields_with_gemini_modern(image)
                    self._log(f"      🤖 Gemini результат: {list(gemini_result.keys())}")
                
                if annotation_method == "ocr":
                    ocr_result = self._extract_fields_with_ocr_modern(image)
                    self._log(f"      👁️ OCR результат: {list(ocr_result.keys())}")
                
                # Применяем консенсус-алгоритм
                multi_extractions = self.quality_enhancer.extract_fields_with_multiple_methods(
                    text=all_text,
                    pdf_text_blocks=self._pdf_text_blocks if hasattr(self, '_pdf_text_blocks') else None,
                    gemini_result=gemini_result,
                    ocr_result=ocr_result
                )
                
                # Получаем консенсус
                consensus_results = self.quality_enhancer.apply_consensus_algorithm(multi_extractions)
                ...
                self._log(f"      ✅ DONUT КОНСЕНСУС: {len(fields)} полей")
                self._log(f"      📊 Качество: {quality_metrics.confidence_score:.1%}")
                self._log(f"      ⚠️ Спорных случаев: {quality_metrics.edge_cases_detected}")
            
            # Fallback к старому методу если качественный недоступен
            elif hasattr(self, '_pdf_has_text_layer') and self._pdf_has_text_layer and self._pdf_text_blocks:
                self._log(f"      📄 Используем данные из текстового слоя PDF...")
                fields = self._extract_fields_from_pdf_text_layer()
                self._log(f"      ✅ Из текстового слоя PDF извлечено полей: {len(fields)}")
            elif annotation_method == "gemini" and self.gemini_processor:
    ...
    def _create_layoutlm_annotation_modern(self,
        ...
        try:
            ...
            try:
                ...
                # 🚀 НОВЫЙ ПОДХОД: Консенсус-алгоритм для максимального качества
                if self.quality_enhancer:
                    self._log(f"     🎯 КАЧЕСТВЕННОЕ ИЗВЛЕЧЕНИЕ: Используем консенсус всех методов...")
                    ...
                    # Конвертируем в структурированные данные
                    structured_data = {
                        field_name: result.final_value 
                        for field_name, result in consensus_results.items()
                    }
                    ...
                    self._log(f"     ✅ КОНСЕНСУС ЗАВЕРШЕН: {len(structured_data)} полей")
                    self._log(f"     📊 Качество: {quality_metrics.confidence_score:.1%} уверенности")
                    self._log(f"     🎯 Точность: {quality_metrics.accuracy:.1%}")
                    self._log(f"     ⚠️ Спорных случаев: {quality_metrics.edge_cases_detected}")
                    
                    # Сохраняем отчет о качестве
                    if hasattr(self, 'temp_output_dir'):
                        quality_report = self.quality_enhancer.generate_quality_report(quality_metrics, consensus_results)
                        self.quality_enhancer.save_quality_report(
                            quality_report, 
                            Path(self.temp_output_dir) / "quality_reports",
                            quality_metrics,
                            consensus_results
                        )
                ...
                # Fallback к старому методу если качественный недоступен
                    ...
                    self._log(f"     📄 Используем данные из текстового слоя PDF...")
                    structured_data = self._extract_fields_from_pdf_text_layer()
                    self._log(f"     ✅ Из текстового слоя PDF извлечено полей: {len(structured_data)}")
                elif annotation_method == "gemini" and self.gemini_processor:
                    self._log(f"     🤖 Извлечение данных через Gemini...")
                    structured_data = self._extract_fields_with_gemini_modern(image)
                    self._log(f"     ✅ Gemini вернул поля: {list(structured_data.keys())}")
